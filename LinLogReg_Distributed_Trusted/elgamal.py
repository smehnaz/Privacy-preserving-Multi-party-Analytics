...
def encode(sPlaintext, iNumBits):
        byte_array = bytearray(sPlaintext, u'utf-16')
...
def decode(aiPlaintext, iNumBits):
        ...
        decodedText = bytearray(b for b in bytes_array).decode(u'utf-16')
...
def generate_keys(iNumBits=256, iConfidence=32):
        ...
        return {u'privateKey': privateKey, u'publicKey': publicKey}
...
def encrypt(key, sPlaintext):
	...
	#cipher_pairs list will hold pairs (c, d) corresponding to each integer in z
        ...
        for pair in cipher_pairs:
                encryptedStr += unicode(pair[0]) + u' ' + unicode(pair[1]) + u' '
...
def decrypt(key, cipher):
        ...
        if (not len(cipherArray) % 2 == 0):
                return u"Malformed Cipher Text"
        for i in xrange(0, len(cipherArray), 2):
	...
	#remove trailing null bytes
        decryptedText = u"".join([ch for ch in decryptedText if ch != u'\x00'])

        return decryptedText

def test():
    keys = generate_keys()
    priv = keys[u'privateKey']
    #print ('priv')
    #print (priv)
    pub = keys[u'publicKey']
    #print ('pub')
    #print (pub)
    message = u"My name is Ryan"
