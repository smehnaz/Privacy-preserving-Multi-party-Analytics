...
class SecSumNodeComm(object):
    ...
    def send_encrypted_random_numbers_to_mediator(self, random_numbers):
        self.comm.request(comm.MSG_RAND_NODE_MEDIATOR, random_numbers)


    def receive_combined_encrypted_random_numbers_from_mediator(self):
        """
            Node receives data to decrypt and shuffle.
        """
        content = self.comm.request(comm.MSG_RAND_MEDIATOR_NODE, "")
        return content


    def send_anonymized_local_results_to_mediator(self, anonymized_local_results):
        self.comm.request(comm.MSG_LOCAL_NODE_MEDIATOR, anonymized_local_results)