...
####################################################################################
    ...
    def receive_encrypted_random_numbers(self):
        """
            Mediator receives public keys from all nodes.
        """
        content = self.comm.reply_all(comm.MSG_RAND_NODE_MEDIATOR, "")
        return content


    def send_combined_encrypted_random_numbers_to_all_parties(self, combined_encrypted_random_numbers, parties, number_of_dimensions):

        content = {}

        # print len(combined_encrypted_random_numbers), number_of_dimensions, len(parties)

        diff = number_of_dimensions * len(parties)

        for i in range(len(parties)):
            encrypted_random_numbers_for_party_i = []
            for ii in range(len(parties)):
                s_index = ii*diff + i*number_of_dimensions
                for j in range(number_of_dimensions):
                    encrypted_random_numbers_for_party_i.append(combined_encrypted_random_numbers[s_index+j])
            content[parties[i]] = encrypted_random_numbers_for_party_i
            # print encrypted_random_numbers_for_party_i, '\n'

        # Send the content
        self.comm.reply(comm.MSG_RAND_MEDIATOR_NODE, content)


    def receive_anonymized_local_results(self):

        content = self.comm.reply_all(comm.MSG_LOCAL_NODE_MEDIATOR, "")
        return content